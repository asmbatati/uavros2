...
`uavros2` dispatches to one of six simulator backends via `simulator:=<name>` on the top-level launch. Status, scope, and install steps per backend:
...
ros2 launch uavros2 sim.launch.py simulator:=gazebo uav:=x500_d435 world:=warehouse
...
Launch:
```bash
ros2 launch uavros2 sim.launch.py simulator:=webots
```
...
Launch:
```bash
ros2 launch uavros2 sim.launch.py simulator:=mujoco uav:=x500
```
...
Launch:
```bash
ros2 launch uavros2 sim.launch.py simulator:=mujoco uav:=x500 arm:=panda
```
...
Launch:
```bash
ros2 launch uavros2 sim.launch.py simulator:=pybullet uav:=x500
```
